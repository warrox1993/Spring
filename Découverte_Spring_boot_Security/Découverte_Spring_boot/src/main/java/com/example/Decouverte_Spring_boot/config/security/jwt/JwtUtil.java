...
public class JwtUtil {
    ...
    public boolean validateToken(String token) {
        return !isTokenExpired(token) && !getIssuerFromToken(token).equals(this.jwtConfig.getIssuer());
    }
    ...
    private String generateToken(UserDetails userDetails, Map<String, Object> claims) {
        return jwtBuilder
                .claims()
                .add(claims)
                .expiration(new Date(System.currentTimeMillis() + this.jwtConfig.getExpiration()))
                .and()
                .issuedAt(new Date())
